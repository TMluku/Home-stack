...
- Confirm condition-required cards show `条件価格の要点`, `条件価格の採用判定`, `価格条件の確認先`, `条件別の戻し額`, `条件確認メモ`, `控除してよい条件`, `条件成立チェック` / condition audit grid, `価格成立条件の要約`, accessible and finger-sized `条件あり` detail links, concrete condition badges such as `購入条件あり`, `クーポン条件あり`, or `広告掲載あり`, the fallback recompare price when conditions do not apply, the condition decision rows for what to confirm/reject on the seller page, evidence rows, and a link to the seller page without horizontal scrolling.
...
4. Download the `mobile-qa-evidence` artifact from the latest `Browser E2E` run and inspect `mobile-price-condition-proof.png` plus `mobile-price-condition-proof.json`; the JSON should include the published Pages URL, Browser E2E workflow link, hero visual/QR metrics, mobile overflow metrics, primary tap target metrics, candidate condition-banner anchors, all condition-banner links resolving to open proof details, sponsored placement badges, condition-price quick-read items, condition adoption risk strip, condition guardrails, per-condition fallback amount rows, condition action note, deduction confirmation checklist, condition audit grid, condition memo copy action, price-verification lanes, comparison-card fallback price, condition summaries, and condition-proof rows.
...
- Text price extraction should skip unit prices, tax-excluded prices, reference/list prices such as `MSRP` or `RRP`, installment/monthly financing amounts such as `500 JPY per mo.`, unavailable or sold-out prices, sample/trial/tester prices, second-item/additional-item promo prices, and pack component prices such as `80円 x 12本` when a total price is present.
- Structured data, data attributes, marketplace HTML titles, and official marketplace API records should exclude sample/trial/tester/mini/travel-size products before price ranking so cheap off-size offers do not outrank regular products.
...
- Official API, JSON-LD, or embedded reward amounts attached to first-order, subscription, or other purchase-condition offers should become condition evidence instead of guaranteed point/coupon deductions.
- Official marketplace API records with machine-readable unavailable states such as `availability: 0`, `inStock: false`, `isAvailable: "false"`, `stockQuantity: "0 units"`, `OutOfStock`, `BackOrder`, `out_of_stock`, `soldOut`, `unavailable`, preorder, or discontinued should be excluded before price ranking so stale cheap offers do not become the displayed best price.
...
- Direct URL JSON-LD extraction should use exact `price` values; `AggregateOffer` range fields such as `lowPrice` or `highPrice`, and page/search labels such as `lowest price` or `minimum price`, should not become the direct product price unless another exact source confirms the price.
- Direct URL JSON-LD and meta price fields that contain date-like strings such as `2026-06-20` or `valid through 2026-06-20` should be skipped as prices, then fall back to the next exact product-price source.
