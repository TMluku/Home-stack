...
| `POST` | `/api/notifications/dispatch` | Dry-run notification delivery by default; with `dryRun: false`, configured LINE jobs are sent through the LINE push API while email/Web Push remain adapter-boundary results. |
...
- Notification dispatch should run as dry-run by default. With `dryRun: false`, unconfigured providers fail with `provider-not-configured`; configured LINE jobs call the LINE Messaging API push endpoint, while configured email/Web Push providers remain adapter-boundary results until real senders are wired in.
