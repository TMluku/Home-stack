...
- `mobile-price-condition-proof.json`: captured URL, viewport, overflow metrics, hero visual/QR metrics, QA checklist count, condition badges, condition-summary rows, price-breakdown rows, seller link, and the assertions covered by the automated pass
