...
- `mobile-price-condition-proof.json`: captured URL, expected published Pages URL, Browser E2E workflow link, viewport, overflow metrics, hero visual/QR metrics, QA checklist count, condition badges, condition quick-read rows, condition guardrails, condition-summary rows, price-breakdown rows, price-verification lanes, condition fallback recompare price, seller link, and the assertions covered by the automated pass
...
12. Open the condition details and confirm the short condition summary, fallback recompare price, decision rows, and evidence rows are readable, with condition labels such as `購入条件あり`, `送料条件あり`, `ポイント条件あり`, or `クーポン条件あり` visible when they apply.
