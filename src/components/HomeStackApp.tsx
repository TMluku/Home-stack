...
import { formatPriceEvidence } from "@/lib/price-evidence";
import {
...
export function HomeStackApp() {
  ...
  async function copyMobileQaTemplate() {
    ...
    const template = [
      `| ${today} | 端末名を記入 | ブラウザ名を記入 | Wi-Fi / 5G | Pass / Fail | Tested URL: ${publicPagesUrl} / Browser E2E: ${browserE2eWorkflowUrl} / mobile-qa-evidence / mobile-price-condition-proof.png / mobile-price-condition-proof.json / 実機スクリーンショット: 実ファイル名を記入 / 条件確認メモ / condition audit grid / 条件別の戻し額 / 広告掲載あり / GitHub Pages実機QA: 横スクロール、価格条件、URLスキャン、共有導線を確認 |`,
      "",
      ...
      "- Sponsored placement condition: 広告掲載あり",
    ].join("\n");
  ...
  return (
    <fieldset className={showConditionProof ? "effective-proof effective-proof--conditional" : "effective-proof"} id={proofId}>
      ...
      {guardrailItems.length > 0 ? (
        <dl className="effective-proof__guardrails" aria-label="価格条件の確認先">
