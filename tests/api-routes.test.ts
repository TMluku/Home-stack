...
describe("API route contracts", () => {
  ...
  it("treats official reward percentage strings as rates instead of yen amounts", async () => {
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async (url) => {
        ...
        return new Response(
          JSON.stringify({
            Items: [
              {
                Item: {
                  itemName: "Percent official Rakuten item",
                  itemPrice: 2000,
                  itemUrl: "https://rakuten.example.test/percent-reward",
                  pointAmount: "10%",
                  couponAmount: "15%",
                },
              },
      ...
      return new Response(
        JSON.stringify({
          hits: [
            {
              name: "Percent official Yahoo item",
              url: "https://shopping.example.test/percent-reward",
              price: 2400,
              point: { amount: "5%" },
              coupon: { amount: "20%" },
            },
    ...
    const response = await searchProducts(
      new Request("http://localhost/api/product-search", {
        method: "POST",
        body: JSON.stringify({ query: "percent official reward" }),
      }),
    );
    const payload = await response.json();
    const rakutenCandidate = payload.candidates.find((candidate: { source: string }) => candidate.source === "rakuten");
    ...
    expect(rakutenCandidate.evidence).toEqual(expect.arrayContaining(["official point value: 200 JPY", "official coupon value: 300 JPY"]));
    expect(rakutenCandidate.evidence).not.toEqual(
      expect.arrayContaining(["official point value: 10 JPY", "official coupon value: 15 JPY"]),
    );
    expect(yahooCandidate.effectivePriceQuote).toMatchObject({
      listPrice: 2400,
      pointValue: 120,
    ...
    expect(yahooCandidate.evidence).toEqual(expect.arrayContaining(["official point value: 120 JPY", "official coupon value: 480 JPY"]));
  ...
  it("ignores coupon-code applied prices when parsing marketplace HTML results", async () => {
    vi.spyOn(globalThis, "fetch").mockImplementation(async (url) => {
      const requestUrl = String(url);
      if (requestUrl.includes("search.rakuten.co.jp")) {
        return new Response(
          `
            <html>
              <body>
                <li>
                  <a href="https://rakuten.example.test/coupon-code-price">Detergent refill coupon code deal</a>
      ...
      return new Response(
        `
          <html>
            <body>
              <article>
                <a href="https://shopping.example.test/promo-code-price">Detergent refill promo code deal</a>
                <span>promo code applied price ¥1,750</span>
                <strong>item price ¥2,080</strong>
              </article>
            </body>
          </html>
    ...
    const response = await searchProducts(
      new Request("http://localhost/api/product-search", {
        method: "POST",
        body: JSON.stringify({ query: "detergent refill coupon code deal" }),
      }),
    );
    const payload = await response.json();
    ...
    expect(payload.candidates).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          url: "https://rakuten.example.test/coupon-code-price",
          price: 1980,
          effectivePriceQuote: expect.objectContaining({
            conditionLabels: expect.arrayContaining(["クーポン条件あり"]),
            conditionRequired: true,
          }),
        }),
        expect.objectContaining({
          url: "https://shopping.example.test/promo-code-price",
          price: 2080,
          effectivePriceQuote: expect.objectContaining({
            conditionLabels: expect.arrayContaining(["クーポン条件あり"]),
            conditionRequired: true,
  ...
  it("ignores expired sale prices when parsing marketplace HTML results", async () => {
    vi.spyOn(globalThis, "fetch").mockImplementation(async (url) => {
      const requestUrl = String(url);
      if (requestUrl.includes("search.rakuten.co.jp")) {
        return new Response(
          `
            <html>
              <body>
                <li>
                  <a href="https://rakuten.example.test/expired-sale">Detergent refill ended sale</a>
                  <span>タイムセール終了 980円</span>
                  <strong>現在価格 1,980円</strong>
                </li>
              </body>
            </html>
          `,
      ...
      return new Response(
        `
          <html>
            <body>
              <article>
                <a href="https://shopping.example.test/deal-ended">Detergent refill deal ended</a>
                <span>deal ended ¥990</span>
                <strong>item price ¥2,080</strong>
              </article>
            </body>
          </html>
    ...
    const response = await searchProducts(
      new Request("http://localhost/api/product-search", {
        method: "POST",
        body: JSON.stringify({ query: "detergent refill ended sale" }),
      }),
    );
    const payload = await response.json();
    ...
    expect(payload.candidates).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ url: "https://rakuten.example.test/expired-sale", price: 1980 }),
        expect.objectContaining({ url: "https://shopping.example.test/deal-ended", price: 2080 }),
      ]),
    );
    expect(payload.candidates.map((candidate: { price?: number }) => candidate.price)).not.toEqual(expect.arrayContaining([980, 990]));
  });
