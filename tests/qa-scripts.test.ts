...
const browserE2eWorkflowWithConditionNote = `${browserE2eWorkflowUrl} / 条件確認メモ / condition audit grid / 条件別の戻し額`;
...
describe("real-device QA gate", () => {
  ...
  it("rejects real-device rows without the per-condition fallback amount check", async () => {
    await withQaFile(
      [
        "| Date | Device | Browser | Network | Result | Notes |",
        "|---|---|---|---|---|---|",
        "| 2026-06-16 | Pixel 8 | Chrome | 5G | Pass | https://tmluku.github.io/Home-stack/ / Browser E2E: https://github.com/TMluku/Home-stack/actions/workflows/e2e.yml / mobile-qa-evidence / mobile-price-condition-proof.png / mobile-price-condition-proof.json / condition action note / condition audit grid / real-device screenshot: phone-price-proof.png |",
      ].join("\n"),
      (filePath) => {
        const result = spawnSync(process.execPath, [scriptPath, filePath], { encoding: "utf8" });
        expect(result.status).toBe(1);
        expect(result.stderr).toContain("条件別の戻し額");
      },
    );
  });
});
